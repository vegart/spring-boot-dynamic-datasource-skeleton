/*
object : ppcwiz common module wrapper
object-name : ppcwiz
module-depth : 1
*/
var ppcwiz = {}; 
...
/*
object : ppcwiz date util wrapper
object-name : ppcwiz
module-depth : 2
*/
ppcwiz.date = {};
...
module-depth : 3
*/
ppcwiz.date.manipulate = function(pDate,pOptions){
	...
	if(typeof pOptions === 'object'){
		if(!ppcwiz.date.isDateValid(pDate)){
			console.log('[ppcwiz.date.manipulate()]failed to parse your date')
			return pDate;
		}else{
			...
			if(ppcwiz.date.isDateValid(finalDate)){
				return finalDate;
			}else{
				console.log('[ppcwiz.date.manipulate()]invalid value for your pOptions parameters.[ppcwiz.date.manipulate()]');
				return pDate;
			}
		}
	}else{
		console.log('[ppcwiz.date.manipulate()]no date manipulation was done. check your pOptions parameters.')
		return pDate;
	}
}

/*
object : ppcwiz form util wrapper
object-name : ppcwiz
module-depth : 2
*/
ppcwiz.form = {};
...
/*
object : ppcwiz form util function
...
module-depth : 3
*/
ppcwiz.form.toJSON = function($param_f){
...
module-depth : 3
*/
ppcwiz.form.validateSimple = function($param_f){
	const $f = $param_f instanceof jQuery ? $param_f : $($param_f);
...
module-depth : 3
*/
ppcwiz.form.bindData = function($param_f,param_data){
...
module-depth : 3
*/
ppcwiz.wrangle.flatten = function(p_arr){
...
explanation : return object without a certain key <EX> ppcwiz.wrangle.removeKey({a: 1,b: 2,c: 3},'a') -> {b:2,c:3}
module-depth : 3
*/
ppcwiz.wrangle.removeKey = function(object,key){
...
explanation : return object without a certain key <EX> ppcwiz.wrangle.removeKeys({a: 1,b: 2,c: 3},['a','b']) -> {c:3}
module-depth : 3
*/
ppcwiz.wrangle.removeKeys = function(object,keyArr){
  const useArr = ppcwiz.wrangle.flatten(keyArr);
	  
  if($.isArray(useArr) & useArr.length > 0){
  	const reducedArr = useArr.slice(1);
    const removed = useArr.shift();
    const newObject = ppcwiz.wrangle.removeKey(object,removed);
    return ppcwiz.wrangle.removeKeys(newObject,reducedArr);
  }else{
  	return object;
  }
}
...
example-usage : ppcwiz.wrangle.mockdata([{a:1,b:'hello'},{a:4,b:'bye'}],10,['a','b'])
module-depth : 3
*/

ppcwiz.wrangle.mockdata = function(
	originalArray,
	mockSize,
	keyArray
){
	return ppcwiz.wrangle.seq_len(mockSize).map(function(intElem){
...
/*
object : ppcwiz component wrapper
object-name : ppcwiz
module-depth : 2
*/
ppcwiz.component = {};
...
/*
object : ppcwiz component function
...
module-depth : 3
*/
ppcwiz.component.datatable = function(p_tableId,p_margin_bottom){
	let $this = $(this);
	...
	$this.create = function(p_data,newOptions){
		...
		let datatableOptions = ppcwiz.wrangle.removeKeys(newOptions,['onRowClick']);
